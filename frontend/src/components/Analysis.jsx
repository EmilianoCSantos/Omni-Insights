...
export default function Analysis() {
  ...
  // Ladda och analysera när filer är valda
  useEffect(() => {
    if (selectedFiles.size === 1) {
      handleSingleFileAnalysis(Array.from(selectedFiles)[0]);
    } else if (selectedFiles.size > 1) {
      handleGroupComparison(Array.from(selectedFiles));
    }
  }, [selectedFiles]);

  // Enkla fil-analys
  const handleSingleFileAnalysis = useCallback(async (fileId) => {
    setIsLoading(true);
    try {
  ...
  // Gruppjämföring
  const handleGroupComparison = useCallback(async (fileIds) => {
    setIsLoading(true);
    try {
      // Dela upp i två grupper: första hälften vs andra hälften
      const midpoint = Math.ceil(fileIds.length / 2);
      const groupA = fileIds.slice(0, midpoint);
      const groupB = fileIds.slice(midpoint);
  ...
  const calculateBeamStats = () => {
    if (!graphData || graphData.length === 0) return null;
    
    const beamOnCount = graphData.filter(d => d.beamOn).length;
    const beamOffCount = graphData.length - beamOnCount;
    const beamOnPercentage = (beamOnCount / graphData.length) * 100;
    ...
    <div style={{ padding: '20px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '20px' }}>
        ...
        <div>
          ...
          <div style={{ marginTop: '20px', backgroundColor: '#f5f5f5', padding: '20px', borderRadius: '8px', maxHeight: '600px', display: 'flex', flexDirection: 'column' }}>
            <h3 style={{ marginTop: 0, paddingBottom: '10px', borderBottom: '1px solid #ddd', marginBottom: '10px' }}>Files ({selectedFiles.size} selected)</h3>
                ...
                {uploadedFiles.map((file) => (
                  <label
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      padding: '10px 15px',
                      backgroundColor: selectedFiles.has(file.id) ? '#e3f2fd' : '#fff',
                      border: selectedFiles.has(file.id) ? '2px solid #007bff' : '1px solid #ddd',
                      borderRadius: '4px',
                      cursor: 'pointer',
                      transition: 'all 0.3s'
                    }}
        ...
        <div>
          ...
          ) : selectedFiles.size === 1 && fileStats ? (
            <>
              {/* SINGLE FILE VIEW */}
              <div style={{ backgroundColor: '#f9f9f9', padding: '15px', borderRadius: '8px', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '14px' }}>
                  ...
                  {(() => {
                    const beamStats = calculateBeamStats();
                    if (!beamStats) return null;
                    return (
                      <div style={{ gridColumn: '1 / -1', backgroundColor: '#fff3cd', padding: '10px', borderRadius: '4px' }}>
                        <strong>📡 Beam On Time:</strong> {beamStats.beamOnCount} / {beamStats.totalPoints} points ({beamStats.beamOnPercentage.toFixed(1)}%)
                      </div>
                    );
                  })()}
                </div>
                    ...
                    <div style={{ backgroundColor: chartBackgroundColor, padding: '15px', borderRadius: '8px' }}>
                      <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={graphData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                          ...
                          <XAxis dataKey="timestamp" tick={{ fontSize: 11 }} angle={-45} textAnchor="end" height={80} />
                    ...
                    <div style={{ backgroundColor: '#f0f8ff', padding: '15px', borderRadius: '8px' }}>
                      <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={graphData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="timestamp" tick={{ fontSize: 11 }} angle={-45} textAnchor="end" height={80} />
                          <YAxis label={{ value: 'Rotation (degrees)', angle: -90, position: 'insideLeft' }} />
                          <Tooltip formatter={(value) => value.toFixed(2)} />
                          <Legend />
                          ...
                          <tbody>
                            {[axisStats.x, axisStats.y, axisStats.z, axisStats.pitch, axisStats.roll, axisStats.yaw].map((stat, idx) => (
                              <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#f5f5f5' }}>
                                ...
                                <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>{stat.mean.toFixed(2)} {stat.unit}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>{stat.maxAbs.toFixed(2)} {stat.unit}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>{stat.stdDev.toFixed(2)} {stat.unit}</td>
          ...
          ) : selectedFiles.size > 1 && comparisonResult ? (
            <>
              {/* GROUP COMPARISON VIEW */}
              <div style={{ marginTop: '40px' }}>
                <h2>Comparison Results ({selectedFiles.size} files)</h2>
                ...
                {activeTab === 'position' && (
                  <div style={{ backgroundColor: '#f0f8ff', padding: '15px', borderRadius: '8px' }}>
                    <ResponsiveContainer width="100%" height={400}>
                      <LineChart
                        data={[
                          { axis: 'Lateral (X)', 'Group 1': parseFloat(comparisonResult.groupA.x_mean), 'Group 2': parseFloat(comparisonResult.groupB.x_mean) },
                          { axis: 'Longitudinal (Y)', 'Group 1': parseFloat(comparisonResult.groupA.y_mean), 'Group 2': parseFloat(comparisonResult.groupB.y_mean) },
                          { axis: 'Vertical (Z)', 'Group 1': parseFloat(comparisonResult.groupA.z_mean), 'Group 2': parseFloat(comparisonResult.groupB.z_mean) }
                        ]}
                      ...
                      >
                        ...
                        <Tooltip formatter={(value) => value.toFixed(2)} />
                        <Legend />
                        <Line type="monotone" dataKey="Group 1" stroke="#0066cc" strokeWidth={2} />
                        <Line type="monotone" dataKey="Group 2" stroke="#00aa00" strokeWidth={2} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                )}
                ...
                {activeTab === 'rotation' && (
                  <div style={{ backgroundColor: '#ffe8e8', padding: '15px', borderRadius: '8px' }}>
                    <ResponsiveContainer width="100%" height={400}>
                      <LineChart
                        data={[
                          { axis: 'Pitch', 'Group 1': parseFloat(comparisonResult.groupA.pitch_mean), 'Group 2': parseFloat(comparisonResult.groupB.pitch_mean) },
                          { axis: 'Roll', 'Group 1': parseFloat(comparisonResult.groupA.roll_mean), 'Group 2': parseFloat(comparisonResult.groupB.roll_mean) },
                          { axis: 'Yaw', 'Group 1': parseFloat(comparisonResult.groupA.yaw_mean), 'Group 2': parseFloat(comparisonResult.groupB.yaw_mean) }
                        ]}
                        margin={{ top: 5, right: 30, left: 0, bottom: 5 }}
                      >
                        <CartesianGrid strokeDasharray="3 3" />
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                      ...
                      <tbody>
                        {[
                          ...
                          <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#f5f5f5' }}>
                            <td style={{ padding: '10px', border: '1px solid #ddd', fontWeight: 'bold' }}>{row.axis}</td>
                            <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>
                              {formatNumber(comparisonResult.groupA[`${row.key}_mean`])} ± {formatNumber(comparisonResult.groupA[`${row.key}_stdDev`])} {row.unit}
                            </td>
                            <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>
                              {formatNumber(comparisonResult.groupB[`${row.key}_mean`])} ± {formatNumber(comparisonResult.groupB[`${row.key}_stdDev`])} {row.unit}
                            </td>
                            <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>
            ...
            <div style={{ backgroundColor: '#f5f5f5', padding: '40px', textAlign: 'center', borderRadius: '8px' }}>
              <p style={{ color: '#666' }}>
                {selectedFiles.size === 0 
                  ? 'Select 1 file to analyze or 2+ files to compare'
                  : 'Loading analysis...'}
              </p>
            </div>
          )}
